...
const RWRN_Plugin_Panel = preload("res://addons/randomnamesgenerator/v_box_container.tscn")
const THE_ICON = preload("res://addons/randomnamesgenerator/icon.png") #icon

var rwrn_plugin_instance
...
func _enter_tree(): #functions starting with an underscore are default functions that we are over-riding by creating them here.
	...
	rwrn_plugin_instance = RWRN_Plugin_Panel.instantiate()

	# Add the main panel to the editor's main viewport
	get_editor_interface().get_editor_main_screen().add_child(rwrn_plugin_instance)
...
func _exit_tree(): # Clean-up of the plugin goes here.
	if rwrn_plugin_instance:
		rwrn_plugin_instance.queue_free()
...
func _make_visible(visible): #If the plugin exists, change visibility flag to true, making it visible
	if rwrn_plugin_instance:
		rwrn_plugin_instance.visible = visible


func _get_plugin_name(): #set the plugin name
	return "Rand Name Gen 1.0"
